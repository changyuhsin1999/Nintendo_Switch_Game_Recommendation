...
st.header("Nintendo Switch Game Recommendation System")
def load_data():
    """
    load and cache data
    :return: game data
    """
    game_data = pd.read_pickle("my_data.pkl")
    game_data = pd.read("./data/switch-games.csv")
    game_data = tfidf_data.set_index('title')
    return tfidf_data
